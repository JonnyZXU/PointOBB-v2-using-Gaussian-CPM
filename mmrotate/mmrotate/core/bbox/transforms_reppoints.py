...
import torch
import math
import cv2  # NEW add this near the top of the file with the other imports
PI = np.pi
...
def points_to_rbox_minarea(points):                                             # NEW FUNCTION
    """
    Convert a set of points around an object to a single oriented box
    using OpenCV's minAreaRect.

    Args:
        points:
            - Tensor of shape (N, K, 2) or (N, 2*K)
              N = number of objects
              K = number of points per object

    Returns:
        Tensor of shape (N, 5): [cx, cy, w, h, angle] for each object,
        where angle is in radians, following the same convention
        as poly2rbox / rbox2poly.
    """
    ...
    # handle (N, 2*K) → (N, K, 2)
    if pts_np.ndim == 2 and pts_np.shape[1] % 2 == 0:
        K = pts_np.shape[1] // 2
        pts_np = pts_np.reshape(pts_np.shape[0], K, 2)
    elif pts_np.ndim != 3 or pts_np.shape[2] != 2:
        raise ValueError("points must be (N, K, 2) or (N, 2*K)")

    rrects = []
    for i in range(pts_np.shape[0]):
        pts_i = pts_np[i].astype(np.float32)

        # OpenCV expects shape (num_points, 1, 2)
        pts_i_cv = pts_i.reshape(-1, 1, 2)

        rect = cv2.minAreaRect(pts_i_cv)
        ...
        rrects.append([cx, cy, w, h, angle_rad])

    rrects = np.asarray(rrects, dtype=np.float32)
    # convert to torch Tensor on the same device as input, if input was Tensor
    if isinstance(points, torch.Tensor):
        rrects = torch.from_numpy(rrects).to(points.device)
